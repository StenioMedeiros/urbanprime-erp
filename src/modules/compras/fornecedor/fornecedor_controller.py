...
from .fornecedor_schema import FornecedorCreate, FornecedorRead, FornecedorUpdate
from .fornecedor_service import FornecedorService

router = APIRouter(prefix="/fornecedores", tags=["Compras - Fornecedores"])
...
@router.get("/{item_id}", response_model=FornecedorRead)
...
def create_item(payload: FornecedorCreate, db: Session = Depends(get_db), _user=Depends(require_permission("compras", "criar"))):
    return service.create(db, payload)


@router.put("/{item_id}", response_model=FornecedorRead)
def update_item(item_id: int, payload: FornecedorUpdate, db: Session = Depends(get_db), _user=Depends(require_permission("compras", "editar"))):
