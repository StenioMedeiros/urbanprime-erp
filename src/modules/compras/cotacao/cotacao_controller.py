...
from .cotacao_schema import CotacaoCreate, CotacaoRead, CotacaoUpdate
from .cotacao_service import CotacaoService

router = APIRouter(prefix="/cotacoes", tags=["Compras - Cotacoes"])
service = CotacaoService()


@router.get("/", response_model=list[CotacaoRead])
...
@router.get("/{item_id}", response_model=CotacaoRead)
...
@router.post("/", response_model=CotacaoRead, status_code=status.HTTP_201_CREATED)
def create_item(payload: CotacaoCreate, db: Session = Depends(get_db), _user=Depends(require_permission("compras", "criar"))):
    return service.create(db, payload)


@router.put("/{item_id}", response_model=CotacaoRead)
def update_item(item_id: int, payload: CotacaoUpdate, db: Session = Depends(get_db), _user=Depends(require_permission("compras", "editar"))):
